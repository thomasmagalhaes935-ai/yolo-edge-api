...
class OptimizedCamera:
    ...
    def _loop(self):
        while self._running.is_set():
            ...
            # Procura o final do primeiro JPEG completo disponível
            end = self._raw.find(b"\xff\xd9")

            if end == -1:
                continue

            # Procura o início do JPEG antes do final encontrado
            ...
            # Remove o JPEG já processado do buffer
            self._raw = self._raw[end + 2:]

            # Decodifica JPEG para BGR
            frame = cv2.imdecode(
...
class RealtimeDetector:
    ...
    def process(self, frame: np.ndarray) -> np.ndarray:
        ...
        - Nos demais frames: reutiliza _last_boxes
